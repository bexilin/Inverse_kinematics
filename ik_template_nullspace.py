...
if __name__ == "__main__":
    ...
    for i in range(5):
        ...
        while 1:
            ...
            delta_q2 = numpy.zeros((7, 1))
            for i in range(len(q)):
                if q[i] < doflimits[0][i] + 0.5:
                    delta_q2[i] = doflimits[0][i] + 0.5 - q[i]
                elif q[i] > doflimits[1][i] - 0.5:
                    delta_q2[i] = doflimits[1][i] - 0.5 - q[i]

            a = 1.0*numpy.matmul((numpy.identity(len(numpy.matmul(Jinv, J)))-numpy.matmul(Jinv, J)),delta_q2)
            delta_q = numpy.matmul(Jinv, delta_x) + numpy.transpose(a)
            delta_q = delta_q[0]
